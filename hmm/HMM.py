...
class HiddenMarkovModel:
    ...
    def unsupervised_learning(self, X, N_iters):
        ...
        for iteration in tqdm(range(1, N_iters + 1)):
            ...
            start_num = [0. for i in range(self.L)]
            ...
            for x in X:
                ...
                for i in range(self.L):
                    sum_p += alphas[1][i] * betas[1][i]

                for i in range(self.L):
                    start_num[i] += alphas[1][i] * betas[1][i] / sum_p
            ...
            for curr in range(self.L):
                self.A_start[curr] = start_num[curr] / len(X)
    ...
    def save(self, filename):
        ...
        f.write("\t".join([str(x) for x in self.A_start]))
        f.write("\n")
        for row in self.A:
...
def unsupervised_HMM(X, n_states, N_iters):
    ...
    HMM = HiddenMarkovModel(A, O, [1 / len(A) for _ in range(len(A))])
...
def load(filename):
    ...
    return HiddenMarkovModel(A, O, A_start)
